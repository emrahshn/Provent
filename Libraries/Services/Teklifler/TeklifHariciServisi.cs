...
namespace Services.Teklifler
{
    ...
    {
        private const string TEKLIF_ALL_KEY = "teklif.all";
        ...
        public IList<TeklifHarici> TümTeklifAl()
        {
            string key = string.Format(TEKLIF_ALL_KEY);
        ...
        public IList<TeklifHarici> TeklifAlPO(string PO)
        {
            var query = _teklifDepo.Tablo.Where(x => x.Po == PO);
            query = query.OrderByDescending(x => x.Tarih);
            return query.ToList();
        }
        public IList<TeklifHarici> TeklifAlIds(List<int> ids)
        {
            List<TeklifHarici> teklifTablo = new List<TeklifHarici>();
            foreach (var t in ids)
            {
                var item = _teklifDepo.Tablo.Where(x => x.Id == t).FirstOrDefault();
                teklifTablo.Add(item);
            }
            //teklifTablo = teklifTablo.OrderByDescending(x => x.Tarih);
            return teklifTablo.ToList();
        }

...
namespace Services.Kongre
{
    ...
    {
        private const string TRANSFER_ALL_KEY = "transfer.all-{0}";
        ...
        public IList<Transfer> TümTransferAl()
        {
            string key = string.Format(TRANSFER_ALL_KEY);
        ...
        public IList<Transfer> TransferAlKongreId(int kongreId)
        {
            string key = string.Format(TRANSFER_ALL_KEY);
            return _önbellekYönetici.Al(key, () =>
            {

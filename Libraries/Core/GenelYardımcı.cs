...
namespace Core
{
    public partial class GenelYardımcı
    {
        public static string AboneMailAdresindenEminOl(string email)
        {
            ...
            {
                throw new TSHata("Email geçerli değil.");
            }
        ...
        private static AspNetHostingPermissionLevel? _güvenSeviyesi;
        public static AspNetHostingPermissionLevel GüvenSeviyesiAl()
        {
            if (!_güvenSeviyesi.HasValue)
            {
                //minimum ayarla
                _güvenSeviyesi = AspNetHostingPermissionLevel.None;

                //maksimum belirle
                foreach (AspNetHostingPermissionLevel güvenSeviyesi in new[] {
                            ...
                            })
                {
                    try
                    {
                        new AspNetHostingPermission(güvenSeviyesi).Demand();
                        _güvenSeviyesi = güvenSeviyesi;
                        break; //mümkün olan en yüksek güvenlik seviyesi belirlendi
                    }
        ...
        public static void SeçenekAyarla(object model, string seçenekAdı, object değer)
        {
            ...
            if (pi == null)
                throw new TSHata("'{1}' model tipinde bir '{0}' seçenek adı bulunamadı.", seçenekAdı, modelTipi);
            if (!pi.CanWrite)
                throw new TSHata("Seçenek adı '{0}' olan '{1}' model tipinin ayarlayıcısı yoktur.", seçenekAdı, modelTipi);
        ...
        public static string MapPath(string yol)
        {
            ...
            {
                //hosted
                return HostingEnvironment.MapPath(yol);
            }

            //not hosted
            string KökKlasör = AppDomain.CurrentDomain.BaseDirectory;
            yol = yol.Replace("~/", "").TrimStart('/').Replace('/', '\\');
            return Path.Combine(KökKlasör, yol);
        }
    }
}
